...
많은 복잡한 시스템과 마찬가지로 Hyperledger Fabric 네트워크는 구조 와 여러 관련 프로세스로 구성된다.

구조 : 사용자 (관리자 등), 조직, 피어, 주문 노드, CA, 스마트 계약 및 애플리케이션을 포함한다.

프로세스 : 이러한 구조가 상호 작용하는 방식. 이들 중 가장 중요한 것은 정책 , 어떤 사용자가 어떤 조건에서 무엇을 할 수 있는지를 결정하는 규칙이다.

 블록 체인 네트워크의 구조를 식별하는 정보와 구조가 상호 작용하는 방식을 제어하는 프로세스가 채널 구성(channel configuration)에 포함된다. 이러한 구성은 채널 구성원에 의해 집합적으로 결정되며 채널 원장에 커밋된 블록에 포함된다. 채널 구성은 configtx.yaml 파일을 입력으로 사용하는 configtxgen이라는 도구를 사용하여 빌드할 수 있습니다. 여기에서 샘플 configtx.yaml 파일을 볼 수 있다.
 
 구성이 블록에 포함되어 있기 때문에 (첫 번째는 채널의 현재 구성을 최신으로 나타내는 최초 블록이라고 함) 채널 구성을 업데이트하는 프로세스 (예를 들어 구성원을 추가하여 구조를 변경하거나 채널 정책 수정)을 구성 업데이트 트랜잭션이라고 한다.

 프로덕션 네트워크에서 이러한 구성 업데이트 트랜잭션은 일반적으로 채널의 초기 구성이 채널의 초기 구성원에 의해 대역 외에서 결정되는 것처럼 대역 외 논의 후 단일 채널 관리자가 제안한다.

이 항목에서는 다음을 수행한다.
 
- 애플리케이션 채널의 전체 샘플 구성을 보여준다.
- 편집 할 수있는 많은 채널 매개 변수에 대해 논의한다.
- 사람이 읽을 수있는 것으로 구성을 가져오고 변환하고 범위를 지정하는데 필요한 명령을 포함하여 채널 구성을 업데이트하는 프로세스를 보여준다.
- 채널 구성을 편집하는 데 사용할 수있는 방법에 대해 토론한다.
- 구성을 다시 포맷하고 승인에 필요한 서명을 얻는 데 사용 된 프로세스를 보여준다.

## 11-2. 업데이트 할 수있는 채널 매개 변수

 채널은 고도로 구성 가능하지만 무한하지는 않습니다. 채널에 대한 특정 사항 (예 : 채널 이름)이 지정되면 변경할 수 없습니다. 그리고 이 항목에서 설명할 매개 변수 중 하나를 변경하려면 채널 구성에 지정된 관련 정책을 충족해야합니다.

이 섹션에서는 샘플 채널 구성을 살펴보고 업데이트 할 수있는 구성 매개 변수를 보여줍니다.

### 11-2-1. 샘플 채널 구성

 어플리케이션 채널의 구성 파일을 가져와서 범위를 지정한 후 어떻게 보이는지 확인하려면 여기를 클릭하여 아래 구성을 확인한다.

예를 들어, 몇 개의 탭을 닫은 상태에서 구성을 살펴 보겠습니다.

이것은 주문자 시스템 채널이 아니라 응용 프로그램 채널의 구성입니다.

이제 구성의 구조가 더 명확해진니다. 구성 그룹 (채널, 애플리케이션 및 주문자)과 각 구성 그룹과 관련된 구성 매개 변수 (다음 섹션에서 자세히 설명)를 볼 수 있지만 조직을 나타내는 MSP가 있는 위치도 확인할 수 있다. 채널 구성 그룹은 Orderer 그룹 구성 값 아래에 있다.

![FireShot Capture 012 - 채널 구성 업데이트 — hyperledger-fabricdocs 마스터 문서_ - hyperledger-fabric readthedocs io](https://user-images.githubusercontent.com/68358404/122496113-30bd9e80-d026-11eb-932a-39e29e7cb413.png)

이제 구성의 구조가 더 명확 해진다. 구성 그룹 (채널, 애플리케이션 및 주문자)과 각 구성 그룹과 관련된 구성 매개 변수 (다음 섹션에서 자세히 설명)를 볼 수 있지만 조직을 나타내는 MSP가있는 위치도 확인할 수 있다. 채널 구성 그룹은 Orderer 그룹 구성 값 아래에 있다.

### 11-2-2. 매개 변수에 대한 추가 정보

이 섹션에서는 구성의 위치와 관련하여 구성 가능한 값에 대해 자세히 살펴본다.

첫째, 구성의 여러 부분에서 발생하는 구성 매개 변수가 있다.

* 정책. 정책은 단순한 구성 값 (mod_policy에 정의 된대로 업데이트 할 수 있음)이 아니라 모든 매개 변수를 변경할 수있는 환경을 정의한다. 자세한 내용은 정책을 확인.

* 기능. 네트워크와 채널이 동일한 방식으로 작업을 처리하도록하여 채널 구성 업데이트 및 체인 코드 호출과 같은 항목에 대해 결정적인 결과를 생성. 결정적 결과가 없으면 채널의 한 피어가 트랜잭션을 무효화하고 다른 피어가이를 검증 할 수 있습니다. 자세한 내용은 기능을 확인.

 채널 / 어플리케이션.
 
* 애플리케이션 채널에 고유 한 구성 매개 변수를 제어한다 (예 : 채널 멤버 추가 또는 제거). 기본적으로 이러한 매개 변수를 변경하려면 대부분의 애플리케이션 조직 관리자의 서명이 필요하다.

* 채널에 조직을 추가. 채널에 조직을 추가하려면 MSP 및 기타 조직 매개 변수를 생성하고 여기 (채널 / 애플리케이션 / 그룹 아래)에 추가해야한다.

조직 관련 매개 변수. 조직에 특정한 모든 매개 변수 (예 : 앵커 피어 또는 조직 관리자의 인증서 식별)를 변경할 수 있다. 이러한 값을 변경하면 기본적으로 대부분의 응용 프로그램 조직 관리자가 필요하지 않고 조직 자체의 관리자만 필요하다.

 채널 / 오더러
 
  오더링 서비스 또는 오더러 시스템 채널에 고유한 구성 매개 변수를 관리하고 대부분의 오더링 조직의 관리자가 필요하다. (기본적으로 오더링 조직은 하나만 있지만 더 추가 할 수 있습니다 (예 : 여러 조직이 오더링  서비스에 노드를 제공하는 경우) ).

* 배치 크기. 이러한 매개 변수는 블록의 트랜잭션 수와 크기를 나타낸다. 어떤 블록도 absolute_max_bytes보다 크거나 블록 내에 max_message_count 개 이상의 트랜잭션으로 표시되지 않는다. preferred_max_bytes 아래에 블록을 구성 할 수 있으면 블록이 조기에 절단되고 이 크기보다 큰 트랜잭션이 자체 블록에 나타난다.

* 일괄 시간 초과. 첫 번째 트랜잭션이 도착한 후 블록을 자르기 전에 추가 트랜잭션을 위해 대기하는 시간이다. 이 값을 줄이면 대기 시간이 향상되지만 너무 많이 줄이면 블록이 최대 용량을 채우지 못하여 처리량이 감소 할 수 있다.

* 블록 유효성 검사. 이 정책은 블록이 유효한 것으로 간주되는 서명 요구 사항을 지정한다. 기본적으로 주문 조직의 일부 구성원의 서명이 필요하다.

* 합의 유형. Kafka 기반 주문 서비스를 Raft 기반 주문 서비스로 마이그레이션하기 위해 채널의 합의 유형을 변경할 수 있다. 자세한 내용은 Kafka에서 Raft로 마이그레이션을 확인.

* Raft 오더링 서비스 매개 변수. Raft 오더링 서비스에 고유 한 매개 변수를 보려면 Raft 구성을 확인.

* Kafka 브로커 (해당하는 경우). ConsensusType이 kafka로 설정되면 브로커 목록은 오더러가 시작시 처음 연결할 Kafka 브로커의 일부 하위 집합 (또는 전체)을 열거한다.

 채널
 
 피어 조직과 오더링 서비스 조직이 모두 동의해야하는 구성 매개 변수를 관리하며 대부분의 애플리케이션 조직 관리자와 오더러 조직 관리자의 동의가 모두 필요하다.
 
* 주문자 주소. 클라이언트가 주문자 브로드 캐스트 및 배달 기능을 호출 할 수있는 주소 목록이다. 피어는 이러한 주소 중에서 임의로 선택하고 블록을 검색하기 위해 이들 주소 사이에서 장애 조치를 수행한다.

* 해싱 구조. 블록 데이터는 바이트 배열의 배열이다. 블록 데이터의 해시는 머클 트리로 계산된다. 이 값은 해당 Merkle 트리의 너비를 지정한다. 당분간이 값은 블록 데이터 바이트 연결의 단순 평면 해시에 해당하는 4294967295로 고정된다.

* 해싱 알고리즘. 블록 체인 블록에 인코딩 된 해시 값을 계산하는 데 사용되는 알고리즘이다. 특히 이는 데이터 해시 및 블록의 이전 블록 해시 필드에 영향을 준다. 이 필드에는 현재 유효한 값 (SHA256)이 하나만 있으며 변경해서는 안된다.


### 11-2-3. 시스템 채널 구성 매개 변수

특정 구성 값은 오더러 시스템 채널에 고유하다.

* 채널 생성 정책. 정의 된 컨소시엄에 대한 새 채널의 애플리케이션 그룹에 대한 mod_policy로 설정 될 정책 값을 정의한다. 채널 생성 요청에 첨부 된 시그니처 세트는 새 채널에서이 정책의 인스턴스화와 비교하여 확인된다. 채널 생성이 승인되었다. 이 구성 값은 orderer 시스템 채널에서만 설정된다.

* 채널 제한. 주문자 시스템 채널에서만 편집 할 수 있다. 주문자가 할당하려는 총 채널 수는 max_count로 지정할 수 있다. 이는 컨소시엄 ChannelCreation 정책이 취약한 사전 프로덕션 환경에서 주로 유용하다.

## 11-3. 구성 편집

채널 구성 업데이트는 개념적으로 간단한 3 단계 작업이다.

1. 최신 채널 구성 가져 오기
2. 수정 된 채널 구성 만들기
3. 구성 업데이트 트랜잭션 생성

그러나 보시다시피이 개념적 단순성은 다소 복잡한 프로세스로 둘러싸여 있다. 결과적으로 일부 사용자는 구성 업데이트를 가져오고, 번역하고, 범위를 지정하는 프로세스를 스크립팅하도록 선택할 수 있다. 사용자는 또한 수동으로 또는 jq와 같은 도구를 사용하여 채널 구성 자체를 수정하는 방법을 선택할 수 있다.

특정 목적을 달성하기 위해 채널 구성 편집을 특별히 다루는 두 개의 튜토리얼이 있다.

채널에 조직 추가 : 기존 채널에 추가 조직을 추가하는 프로세스를 보여준다.
채널 기능 업데이트 : 채널 기능을 업데이트하는 방법을 보여준다.

이 항목에서는 구성 업데이트의 최종 목표와 관계없이 채널 구성을 편집하는 프로세스를 보여준다.

### 11-3-1. 구성 업데이트를위한 환경 변수 설정

샘플 명령을 사용하기 전에 배포를 구성한 방식에 따라 달라지는 다음 환경 변수를 내 보내야한다. 채널 구성 업데이트는 업데이트되는 채널의 구성에만 적용되므로 채널 이름 CH_NAME은 업데이트되는 모든 채널에 대해 설정되어야한다 (구성이 구성에 복사되는 주문 시스템 채널은 예외). 기본적으로 애플리케이션 채널).

CH_NAME : 업데이트중인 채널의 이름이다.

TLS_ROOT_CA : 업데이트를 제안하는 조직의 TLS CA 루트 CA 인증서에 대한 경로이다.

CORE_PEER_LOCALMSPID : MSP의 이름이다.

CORE_PEER_MSPCONFIGPATH : 조직의 MSP에 대한 절대 경로이다.

ORDERER_CONTAINER : 주문 노드 컨테이너의 이름. 주문 서비스를 대상으로 할 때 주문 서비스의 모든 활성 노드를 대상으로 할 수 있습니다. 귀하의 요청은 자동으로 리더에게 전달된다.

 참고: 이 항목에서는 가져오고 수정되는 다양한 JSON 및 protobuf 파일 (config_block.pb, config_block.json 등)의 기본 이름을 제공한다. 원하는 이름을 자유롭게 사용할 수 있다. 그러나 각 구성 업데이트가 끝날 때 돌아가서 이러한 파일을 지우지 않는 한 추가 업데이트를 수행 할 때 다른 파일을 선택해야한다.
 
### 11-3-2. 구성 가져오기 및 번역
 
 채널 구성 업데이트의 첫 번째 단계는 최신 구성 블록을 가져 오는 것입니다. 이것은 3 단계 프로세스입니다. 먼저 채널 구성을 protobuf 형식으로 가져와 config_block.pb라는 파일을 만듭니다.
 
```
peer channel fetch config config_block.pb -o $ORDERER_CONTAINER -c $CH_NAME --tls --cafile $TLS_ROOT_CA
```

다음으로 채널 구성의 protobuf 버전을 JSON 버전으로 변환

```
configtxlator proto_decode --input config_block.pb --type common.Block --output config_block.json
```

마지막으로 구성에서 불필요한 모든 메타 데이터의 범위를 지정하여 더 쉽게 읽을 수 있다. 이 파일을 원하는대로 호출 할 수 있지만이 예에서는 config.json이라고 한다.

```
jq .data.data[0].payload.data.config config_block.json > config.json
```

 이제 modified_config.json이라는 config.json의 사본을 만들어 보겠다. 이후 단계에서 config.json과 modified_config.json 간의 차이를 계산하는데 사용할 것이므로 config.json을 직접 편집하지 않는다.
...
이 시점에서 구성을 수정하는 방법에 대한 두 가지 옵션이 있다.

선택한 텍스트 편집기를 사용하여 modified_config.json을 열고 편집한다. 편집기가없는 컨테이너에서 파일을 복사하고 편집 한 다음 컨테이너에 다시 추가하는 방법을 설명하는 온라인 자습서가 있다.

구성에 편집 내용을 적용하려면 jq를 사용 해야 한다.
구성을 수동으로 편집하거나 jq를 사용하도록 선택하는지 여부는 사용 사례에 따라 다르다. jq는 간결하고 스크립팅이 가능하기 때문에 (동일한 구성 업데이트가 여러 채널에 적용될 때 이점) 채널 업데이트를 수행하는 데 권장되는 방법이다. jq를 사용하는 방법에 대한 예는 capabilities.json이라는 기능 구성 파일을 활용하는 여러 jq 명령을 보여주는 채널 기능 업데이트를 확인한다. 채널의 기능 이외의 것을 업데이트하는 경우 그에 따라 jq 명령과 JSON 파일을 수정해야한다.

### 11-3-4. 구성 다시 인코딩 및 제출

 구성 업데이트를 수동으로 수행하든 jq와 같은 도구를 사용하든 관계없이 이제 이전 구성과 새 구성 간의 차이를 계산하는 단계와 함께 구성을 가져오고 범위를 역으로 지정하기 위해 실행 한 프로세스를 실행해야한다. 승인 할 채널의 다른 관리자에게 구성 업데이트를 제출한다.

먼저 config.json 파일을 protobuf 형식으로 되돌려 config.pb라는 파일을 만든다. 그런 다음 modified_config.json 파일로 동일한 작업을 수행한다. 그런 다음 두 파일의 차이를 계산하여 config_update.pb라는 파일을 만든다.
...
이제 이전 구성과 새 구성의 차이를 계산 했으므로 구성에 변경 사항을 적용 할 수 있다.
...
구성 업데이트 트랜잭션은 원래 구성과 수정 된 구성의 차이를 나타내지  주문 서비스는 전체 채널 구성으로 변환한다.

### 11-4. 필요한 서명 받기

새구성 protobuf 파일을 성공적으로 생성 한 후에는 일반적으로 (항상은 아니지만) 다른 조직의 서명을 요구하여 변경하려는 모든 항목에 대한 관련 정책을 충족해야한다.

참고 : 애플리케이션에 따라 서명 컬렉션을 스크립팅 할 수 있다. 일반적으로 귀하는 항상 필요한 것보다 더 많은 서명을 수집 할 수 있다.

이러한 서명을 얻는 실제 프로세스는 시스템 설정 방법에 따라 다르지만 두 가지 주요 구현이 있다. 현재 Fabric 명령 줄은 기본적으로 "pass it along"시스템으로 설정되어 있다. 즉, 구성 업데이트를 제안하는 조직의 관리자가 업데이트를 서명해야하는 다른 사람 (일반적으로 다른 관리자)에게 보낸다. 이 관리자는 구성을 제출할 충분한 서명이있을 때까지 서명 (또는 서명하지 않음)하고 다음 관리자에게 전달하는 식이다.

이는 단순함의 장점이 있다. 서명이 충분할 때 마지막 관리자는 구성 트랜잭션을 간단히 제출할 수 있습니다 (Fabric에서 피어 채널 업데이트 명령은 기본적으로 서명을 포함합니다). 그러나이 프로세스는 "통과"방법이 시간이 많이 소요될 수 있으므로 소규모 채널에서만 실용적이다.

다른 옵션은 채널의 모든 관리자에게 업데이트를 제출하고 충분한 서명이 돌아올 때까지 기다리는 것이다. 그런 다음 이러한 서명을 함께 연결하여 제출할 수 있다. 이로 인해 구성 업데이트 (서명자별로 파일을 처리하도록 강제)를 생성 한 관리자에게는 삶이 조금 더 어려워지지만 패브릭 관리 애플리케이션을 개발하는 사용자에게는 권장되는 워크 플로이다.
...
```
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --channelID mychannel --name asset --version 1.0 --sequence 1 --tls --cafile ${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem --peerAddresses localhost:7051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt --peerAddresses localhost:9051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt
```
