...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Orderer4 Org Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orderer4.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Orderer5 Org Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orderer5.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    ...
    infoln "Creating Orderer Org2 Identities"

    createOrderer2

    infoln "Creating Orderer Org3 Identities"

    createOrderer3

    infoln "Creating Orderer Org4 Identities"

    createOrderer4

    infoln "Creating Orderer Org5 Identities"

    createOrderer5

  fi

  infoln "Generating CCP files for Org1 and Org2 and Org3 and Org4 and Org5"
